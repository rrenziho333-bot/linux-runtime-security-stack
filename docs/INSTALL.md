...
Go 依赖需在有网环境预先拉到模块缓存（部署脚本 `GOPROXY=off` 离线构建）。
这一步要在 clone 项目之后做（需要项目的 `go.mod`），所以放到 §5 克隆之后，本节只装工具链即可。
...
**拉取 Go 依赖（仅完整模式需要，降级模式跳过）：** 现在已在项目根目录（有 `go.mod`），
用你的登录用户（部署时也是这个用户编译）拉一次依赖到模块缓存。需在有网环境做：
...
- 顶部"组件流水线"中 Falco / TSA 显示 `active`（绿色）；BPF LSM 项完整模式为 `active`，
  降级模式为非绿（未启动）属正常；
