...
回到项目目录执行 **4.3**。网页自动刷新，打开“本次证据”链接可看到“验收事件，不计实际风险”。独立扣分结果在终端显示。

只想手动触发同一规则时，可执行以下命令；只入库、不改变实际分数，完整验收使用 4.3：
...
| 情况 | 应看到的结果 |
|---|---|
| 执行 4.3 | 实际告警入库；独立库 100 → 99，重放相同证据不再扣分 |
| 再次执行 4.3 | 新验收证据入库；不累计影响主系统分数 |
| 非验收规则命中 | 按上下文、规则权重和弱线索预算计入实际风险 |

其他进程的告警和风险到期仍可改变实际分数。模拟其他攻击规则应在独立验证环境进行，不按进程名或测试文件夹名称自动免扣分。不要清库或修改报告时间来制造满分。
