...
- **CentOS**：官方 `kernel` 包默认未开 `CONFIG_BPF_LSM`，且 CentOS 7 内核偏旧（≥5.7 才有 BPF LSM，建议至少 CentOS 8 / Stream 9）；可考虑启用 ELRepo 的 `kernel-lt`/`kernel-ml` 或自行编译。
...
- CentOS 7 内核（3.x）**太旧，连 BPF LSM 都没有**（需 ≥5.7），务必用 CentOS 8 / Stream 9 或以上。
- CentOS 8 / Stream 9 官方内核较新，但 `CONFIG_BPF_LSM` 仍可能未开——以 1.1 实测为准。
- CentOS 通常**已开** `CONFIG_DEBUG_INFO_BTF`。
