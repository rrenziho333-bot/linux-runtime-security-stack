...
| `Monitor specific file access` | 实际 0；独立验收 1 | 仅独立验收，15 分钟 | 专用演示文件，不污染实际评分 |
...
旧事件按新上下文重算当前风险，但历史扣分不改写；页面证据详情显示当前策略说明。例外不续期其他可疑证据。撤下的规则和专用验收规则不贡献实际风险；过期仅表示离开观察窗口，不表示已处置。priority 和 MITRE 标签不自动决定分值；未配置规则仅记录。
