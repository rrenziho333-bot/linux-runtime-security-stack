...
适用普通 Ubuntu 主机，不是完整容器/云平台方案；Web 三条只在对应服务活动时触发。`89-host-profile.yaml` 控制检测开关，`tsa/policy_config.yaml` 控制分值。5 分为需上下文的审计线索、10 分为敏感变更/可疑行为、15～20 分为较具体的高风险特征；演示规则单独约定为 10 分，便于观察真实计分，都不是攻击成功判定。
...
| `Monitor specific file access` | 10 | 15 分钟 | 演示文件访问，参与实际评分；用于可视化验收，不表示确认攻击 |

同一规则当前只取未过期证据的最高风险值，重复报警只续期；运行时分最低为 0。上述矿工名称、Stratum 参数、删除工具名称、pkexec 无参数和 GLIBC_TUNABLES 五类弱线索合计最多 15 分，其他规则不受此预算限制。页面分别显示规则风险值和当前贡献；按风险值从高到低分配预算，同值按规则名排序，不表示攻击次数。预算可在 `runtime_rules.signal_budget` 调整。

`context_advisories` 仅对列出的 VMware、GDM、Ubuntu 更新组件的成功只读操作免扣分；必须同时满足 root、宿主机、绝对程序路径、父进程、完整命令和指定文件。缺字段、改变路径、写入、失败操作仍按原规则计分。证据保留，并注明上下文；这些例外不是对程序完整性或未被利用的证明。不具备足够上下文的 Lynis 子进程告警仍需复核，不按父进程名字统一放行。

旧事件按新上下文重算当前风险，但历史扣分不改写；页面证据详情显示当前策略说明。例外不续期其他可疑证据。撤下的规则不贡献实际风险；旧版标记为 `test_event` 的验收记录不追溯扣分，新触发的演示告警实际计分。过期仅表示离开观察窗口，不表示已处置。priority 和 MITRE 标签不自动决定分值；未配置规则仅记录。

**看分数时区分两本账：**“当前运行时扣分明细”按未过期风险重算，合计最多扣 100 分；事件列表是入库当时的新增扣分，不能把历史记录直接求和当成当前风险。新事件会注明同规则已覆盖、弱线索预算封顶或总分下限；旧事件未保存的细分原因不会事后猜测。
...
<details>
<summary>展开精简前的 82 条规则解析（不是当前启用清单）</summary>
